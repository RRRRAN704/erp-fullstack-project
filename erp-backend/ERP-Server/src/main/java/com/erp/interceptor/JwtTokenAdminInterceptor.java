...
public class JwtTokenAdminInterceptor implements HandlerInterceptor {
    ...
    //It checks whether the incoming request contains a valid JWT.
    // If the token is valid, the request proceeds; otherwise, it returns 401 Unauthorized.
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (!(handler instanceof HandlerMethod)) {
            //if this method is not a controller method then allow it to pass
            return true;
        }
