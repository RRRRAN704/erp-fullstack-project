...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    //regular expression: 8-20 digits, including letters and digits
    ...
    public boolean resetPasswordByEmail(ResetPasswordDTO resetPasswordDTO) {
        try {
            ...
            //2. check if new password matches the required format
            String rawPassword = resetPasswordDTO.getNewPassword();
            if(rawPassword == null || !rawPassword.matches(PASSWORD_PATTERN)) {
                log.warn("Password strength check failed for email:{}", resetPasswordDTO.getEmail());
                return false;
            }


            // 3. encrypt new passwords
            String encryptedNewPassword = passwordEncoder.encode(rawPassword);
